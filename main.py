...
def printScreen():
  ...
  if guessesLeft <= 3:# if the player has less than 4 guesses left, they get a hint. The hint is just the definition of the word pulled from the PyDictionary module
  ...
  if len(wrongGuesses) > 0:
    print("Incorrect Guesses:", ' '.join(wrongGuesses))  #gets rid of brackets, parenthesis, and commas when printing
...
while True:
  ...
  #If you want to use this, you must also use the findIndex() function we made on line 126 because a lot of the words have the same letter more than once and using .index() or .find() will not work. 
  ...
  while True:
    ...
    # if the word chosen doesn't pass all of the following checks it will be rerolled
  ...
  guessesLeft = 7
  # create invalid count so an error message is displayed on the next turn if they do something they aren't supposed to do
  invalidCount = 0
